...
For each subject (row) in the data, the difference is:
$$ t_{difference}^{(i)} = t_{incongruent}^{(i)} - t_{congruent}^{(i)} $$
Thus the null hypothesis is:
$$ H_0 : \mu_{difference} = 0 $$
$$ \mu_{difference} = \frac{\sum_{i=1}^N t_{difference}^{(i)}}{N}  $$
