...
ggplot(mean_sales_by_store_and_promo, aes(x=Store,y=Sales,fill=factor(Promo))) +
...
qplot(increase_ratio,binwidth=0.05)
```

The mean is around 2 which is to say that, on average, total sales on promotion days are twice that of non-promotion days. There is seemingly a single store whose average with promotion is less!

```{r}
with_promo[increase_ratio<1,]
without_promo[increase_ratio<1,]

```

Does the presence of regular monthly promotions (Promo2) have any effect on the efficacy of daily promotions (Promo)?
...
ggplot(open,aes(x=Customers,y=Sales)) +
  geom_point(aes(color=StoreType)) + scale_x_log10() + scale_y_log10()
```
...
#### Let's now turn to the competition. Does presence of nearby competition decrease total customers served?

```{r}

ggplot(open, aes(x=CompetitionDistance, y=Customers,color=Assortment)) +
  stat_summary(fun.y='sum', geom='point') + scale_x_log10() + scale_y_log10()

```

Hard to see any trends there. Let's look at the mean instead of the total.

```{r}

ggplot(open, aes(x=CompetitionDistance, y=Customers,color=Assortment)) +
  stat_summary(fun.y='mean', geom='point') + scale_x_log10() + scale_y_log10()

```

There seems to be a somewhat clearer inverse linear trend there. I think competition distance may have more of an effect on public holidays? Let's see.

```{r}
open_onholiday <- subset(open, StateHoliday != 'Regular')


ggplot(open_onholiday, aes(x=CompetitionDistance, 
                           y=Customers,color=Assortment)) +
  stat_summary(fun.y='mean', geom='point') +
  scale_x_log10() +scale_y_log10() + facet_wrap(~StateHoliday)

```

Perhaps there is a clearer negative linear relationship there? Let's look at the correlation coefficient for all three relationships and ignore missing values.
...
cor(stores$TotalCustomers,stores$CompetitionDistance, use='complete.obs')
cor(stores$MeanCustomers,stores$CompetitionDistance, use='complete.obs')
cor(stores$MeanCustomersPublicHoliday,stores$CompetitionDistance, use='complete.obs')
```
